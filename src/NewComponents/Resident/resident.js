					...
					<div className={cn(styles.groupBtnRight)}>
						<div className={cn(styles.selectbtn)}>
							...
							>
								...
								<option value="formProcessing">{'Hình thức xử lý'}</option>
					...
					<table className={cn(styles.tableContent)} cellSpacing="0">
						<thead>
							<tr>
								...
								<th className={cn(styles.host)}>{'Chủ nhà trọ/chủ hộ'}</th>
								<th className={cn(styles.fullName)}>{'Họ tên người được kiểm tra'}</th>
								<th className={cn(styles.citizenNumber)}>{'Số CCCD'}</th>
								<th className={cn(styles.violationer)}>{'Người vi phạm'}</th>
								<th className={cn(styles.formProcessing)}>{'Hình thức xử lý'}</th>
							...
							<tbody>
								{mainData.map((data, index) => (
									<tr key={index}>
										...
										<td onClick={() => handleEdit(data)} className={cn(styles.contentDate)}>
											{data?.Date ? formatDate(String(data.Date)) : ''}
										</td>
										<td onClick={() => handleEdit(data)} className={cn(styles.contentAddress)}>
											{data?.Address ? String(data.Address) : ''}
										</td>
										<td onClick={() => handleEdit(data)} className={cn(styles.contentHost)}>
											{data?.Host ? data.Host : ''}
										</td>
										...
										<td onClick={() => handleEdit(data)} className={cn(styles.contentViolationer)}>
										...
										<td onClick={() => handleEdit(data)} className={cn(styles.contentPoliceCheck)}>
						...
						{loading && (
							<tbody>
								<tr>
									<td style={{ textAlign: 'center' }} colSpan={10} className={cn(styles.loadingArea)}>
										<LoadingTable />
									</td>
								</tr>
							</tbody>
